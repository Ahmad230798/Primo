...
class _AddEditAddressSheetState extends State<AddEditAddressSheet> {
  ...
  void initState() {
    ...
    _descController = TextEditingController(text: widget.address?.description ?? '');
    final initialLat = widget.address?.locationLat?.isNotEmpty == true ? widget.address!.locationLat! : '24.7136';
    final initialLng = widget.address?.locationLng?.isNotEmpty == true ? widget.address!.locationLng! : '46.6753';
    _latController = TextEditingController(text: initialLat);
    _lngController = TextEditingController(text: initialLng);
    _isLocationSelected = widget.address?.locationLat?.isNotEmpty == true && widget.address?.locationLng?.isNotEmpty == true;
  }
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text(
                        ...
                        style: AppTextStyle.font20.copyWith(fontWeight: FontWeight.w700),
                      ),
                      IconButton(
                  ...
                  16.verticalSpace,
                  Text("اسم العنوان (مثل: المنزل، العمل)", style: AppTextStyle.font14.copyWith(fontWeight: FontWeight.w600)),
                  8.verticalSpace,
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r), borderSide: const BorderSide(color: AppColors.formBorder)),
                      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r), borderSide: const BorderSide(color: AppColors.formBorder)),
                    ),
                    validator: (val) => val == null || val.trim().isEmpty ? "الرجاء إدخال اسم العنوان" : null,
                  ),
                  16.verticalSpace,
                  Text("تفاصيل العنوان (الشارع، الحي، رقم المبنى)", style: AppTextStyle.font14.copyWith(fontWeight: FontWeight.w600)),
                  8.verticalSpace,
                  TextFormField(
                    ...
                    validator: (val) => val == null || val.trim().isEmpty ? "الرجاء إدخال تفاصيل العنوان" : null,
                  ),
                  16.verticalSpace,
                  Text("موقع العنوان على الخريطة (إجباري)", style: AppTextStyle.font14.copyWith(fontWeight: FontWeight.w600)),
                  8.verticalSpace,
                  InkWell(
                    onTap: () async {
                      final selectedLatLng = await Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (_) => MapPickerScreen(
                            initialLat: double.tryParse(_latController.text) ?? 24.7136,
                            initialLng: double.tryParse(_lngController.text) ?? 46.6753,
                          ),
                        ),
                      );
                      if (selectedLatLng != null) {
                        setState(() {
                          _latController.text = selectedLatLng.latitude.toString();
                          _lngController.text = selectedLatLng.longitude.toString();
                              ...
                              children: [
                                Text(
                                  ...
                                  style: AppTextStyle.font14.copyWith(
                                    ...
                                    color: _isLocationSelected ? Colors.green.shade800 : AppColors.textMain,
                                  ),
                                ),
                                4.verticalSpace,
                                Text(
